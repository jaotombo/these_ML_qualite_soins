...
RESULT_PATH = "F:/Dropbox/Travaux_JAOTOMBO/These_Sante_Publique/Projets/Duree_Sejour/Results" 
REPEAT = 100
...
data = pd.read_csv(DATA_PATH+"/sejour_sans_obstetrique&severitenondeterminee.csv", low_memory=False, index_col=0)
...
svm_accuracy = []
svm_impMat = []
svm_fpr = pd.DataFrame({'A' : []})
svm_tpr =pd.DataFrame({'A' : []})
svm_thres = pd.DataFrame({'A' : []})
...
#--------------------
    ...
    lr_imp = permutation_importance(lr, X_val, y_val, scoring='roc_auc')
...
#--------------------
    ...
    best_en = LogisticRegression(C= 0.4393970560760795, penalty= 'l2')
...
#--------------------
    ...
    best_ct = DecisionTreeClassifier(criterion = 'gini', max_depth= None, max_features= 8, min_samples_leaf= 14)
...
#--------------------
    ...
    best_gb = GradientBoostingClassifier(subsample= 1, n_estimators= 300, max_features= 'sqrt', max_depth= 4)
...
#------------------------
    ...
    best_svm = SVC(probability= True, kernel= 'poly', gamma= 0.001, C= 1)
   ...
   # Fit the model to the training data
    ...
    fpr, tpr, thres = roc_curve(y_val, svm_pred_proba)
    
    # Compute true positive and false positive rate
    svm_fpr = pd.concat([pd.DataFrame(fpr)], axis=1)
    svm_tpr = pd.concat([pd.DataFrame(tpr)], axis=1)
    svm_thres = pd.concat([pd.DataFrame(thres)], axis=1)
...
svm_fpr_mean = svm_fpr.mean(axis=1)
svm_tpr_mean = svm_tpr.mean(axis=1)
svm_thres_mean = svm_thres.mean(axis=1)

svm_roc_mean = pd.DataFrame({"fpr":svm_fpr_mean, "tpr":svm_tpr_mean, "threshold":svm_thres_mean})

svm_accuracy_data = pd.DataFrame({'Accuracy':svm_accuracy})
svm_auc_data = pd.DataFrame({'AUC':svm_auc})
    
# Compute and print mean AUC score
svm_meanAccuracy = np.mean(svm_accuracy)
print("'\n' average Accuracy for the Support Vector Machine: {:.3f}".format(svm_meanAccuracy))
svm_meanAuc = np.mean(svm_auc) 
print("'\n' average AUC for the Support Vector Machine: {:.3f}".format(svm_meanAuc))
...
svm_scoreData = pd.DataFrame( {'Mean Accuracy':[svm_meanAccuracy], 'Mean AUC':[svm_meanAuc],
                              'Test Accuracy':[svm_test_accuracy], 'Test AUC':[svm_test_auc]})

# Export results
svm_roc_mean.to_excel(RESULT_PATH+"/svm_roc_mean.xlsx")
svm_scoreData.to_excel(RESULT_PATH+"/svm_perf_mean.xlsx")
svm_result.to_excel(RESULT_PATH+"/svm_result.xlsx")
svm_accuracy_data.to_excel(RESULT_PATH+"/svm_accuracy.xlsx")
svm_auc_data.to_excel(RESULT_PATH+"/svm_auc.xlsx")
...
all_accuracy = pd.concat([lr_accuracy_data, en_accuracy_data, ct_accuracy_data,
                         rf_accuracy_data, gb_accuracy_data, svm_accuracy_data, nnet_accuracy_data], axis=1)
all_accuracy.columns =  ["LR","EN","CT","RF","GB","SVM","NNET"]

all_auc = pd.concat([lr_auc_data, en_auc_data, ct_auc_data,
                         rf_auc_data, gb_auc_data, svm_auc_data, nnet_auc_data], axis=1)
all_auc.columns =  ["LR","EN","CT","RF","GB","SVM","NNET"]
...
def ind_ttest(data) :
    ...
    col_names = ["LR","EN","CT","RF","GB","SVM","NNET"]
    ttest_stats = []
    ttest_pvalue = []
    effsize=[]
    row_names=[]
