...
#=================================================================================
# Cette fonction calcule l'importance des variables pour la régression logistique
#=================================================================================
lr_Imp_fct = function (trainSet, outcomeName) {
  # base train
  # Calcul de la contribution de chaque modalité après disjonction totale
...
# Sortie : vecteur avec le nom des prédicteurs en ligne


#==========================================================================
# Cette fonction calcule l'importance des variables en fonction du modèle
#==========================================================================
...
# Sortie : vecteur avec nom des prédicteurs


#=======================================================================
# Cette fonction calcule l'AUC sous le ROC avec la méthode de trapèzes
#=======================================================================
...
#####################################
# Apprentissage et Test à répétition
#####################################
...
for (i in 1:repetition) {
  ...
  print(c("nombre total de répétitions", repetition))

  indice <- createDataPartition(train[,outcomeName], p=0.7, list=FALSE)
  trainSet = train[indice,]
  testSet = train[-indice,]
  
  print(c("répétition #", i, "LR"))
  #=================================================================================
  # Cette partie calcule la performance moyen d'un modèle LR avec 
  # les données pour calculer les tfp, tvp, ROC et importance sur plusieurs tirages
  #=================================================================================
  ...
  lrpred = predict(model_lr1, testSet, type="prob")
  ...
  lrimp = cbind(lrimp,importance(model_lr1,trainSet,predictors,outcomeName)[[1]])

  print(c("répétition #", i, "CART"))
  #=================================================================================
  # Cette partie calcule la performance moyen d'un modèle CART avec 
  # les données pour calculer les tfp, tvp, ROC et importance sur plusieurs tirages
  #=================================================================================  
  model_cart1 = train(trainSet[,predictors],trainSet[,outcomeName],method='rpart',
                    trControl=fitControl, metric="ROC",  tuneLength=10)
  
  cartpred = predict(model_cart1, testSet, type="prob")
  ...
  cartimp = cbind(cartimp,importance(model_cart1,trainSet,predictors,outcomeName,useModel=TRUE)[[1]])
  
  print(c("répétition #", i, "GBM"))
  #=================================================================================
  # Cette partie calcule la performance moyen d'un modèle GBM avec 
  # les données pour calculer les tfp, tvp, ROC et importance sur plusieurs tirages
  #=================================================================================
  model_gbm1 = train(trainSet[,predictors],trainSet[,outcomeName],method='gbm',
                    trControl=fitControl, metric="ROC",  tuneLength=10)
  
  gbmpred = predict(model_gbm1, testSet, type="prob")
  ...
  gbmimp = cbind(gbmimp,importance(model_gbm1,trainSet,predictors,outcomeName,useModel=TRUE)[[1]])
  
  print(c("répétition #", i, "RF1"))
  #=================================================================================
  # Cette partie calcule la performance moyen d'un modèle RF1 avec 
  # les données pour calculer les tfp, tvp, ROC et importance sur plusieurs tirages
  #=================================================================================
  ...
  rf1pred = predict(model_rf1, testSet, type="prob")
  ...
  rf1imp = cbind(rf1imp,importance(model_rf1,trainSet,predictors,outcomeName,useModel=TRUE)[[1]])
  
  print(c("répétition #", i, "RF2"))
  #=================================================================================
  # Cette partie calcule la performance moyen d'un modèle RF2 avec 
  # les données pour calculer les tfp, tvp, ROC et importance sur plusieurs tirages
  #=================================================================================
  model_rf2 = train(trainSet[,predictors],trainSet[,outcomeName],method='ranger',
                    trControl=fitControl, metric="ROC", num.trees=500,  tuneLength=5,importance="impurity")
  
  rf2pred = predict(model_rf2, testSet, type="prob")
  ...
  rf2imp = cbind(rf2imp,importance(model_rf2,trainSet,predictors,outcomeName,useModel=TRUE)[[1]])
  
  print(c("répétition #", i, "NNET"))
  #=================================================================================
  # Cette partie calcule la performance moyen d'un modèle NNET avec 
  # les données pour calculer les tfp, tvp, ROC et importance sur plusieurs tirages
  #=================================================================================
  model_nnet1 = train(trainSet[,predictors],trainSet[,outcomeName],method='nnet',
                    trControl=fitControl, metric="ROC", maxit=1000,  tuneLength=5)
  
  nnetpred = predict(model_nnet1, testSet, type="prob")
  ...
  nnetimp = cbind(nnetimp,importance(model_nnet1,trainSet,predictors,outcomeName,useModel=FALSE)[[1]])
